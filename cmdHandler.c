...
void usb_decode(void)
{
	...
	{
	// Generic commands
	case BM_GET_PRODUCT_ID:
		usb_buffer[PACKET_BYTE_FIRST_DATA + 3] = (u8)(PRODUCT_ID >> 24);
		...
		break;
	case BM_GET_STATUS:
		usb_buffer[PACKET_BYTE_FIRST_DATA] = system_status; // + 1;
		break;
	...
	case BM_GET_BLUEROBIN_ID:
		...
		break;
	case BM_SET_HEARTRATE:
		HeartRate_u8 = usb_buffer[PACKET_BYTE_FIRST_DATA];
		break;
	case BM_SET_SPEED:
		Speed_u8     = usb_buffer[PACKET_BYTE_FIRST_DATA];
  ...
  usb_sendack = sendDataBack;
  usb_buffer[PACKET_BYTE_CMD] = status;
}
